...
(optional scaler) → regression model → simulated Tc in Kelvin.  The user
therefore only needs to supply a chemical formula; no manual preprocessing is
required.
...
BEST_CSV     = PROJECT_ROOT / "results" / "sim_tc_best_by_dataset.csv"
...
def _load_best_model_tags() -> "dict[str, str]":
    """Return {onnx_base_name: 'best for <Dataset>'} from sim_tc_best_by_dataset.csv.
...
def main() -> None:
    ...
    if args.list:
        ...
        if not groups:
            ...
            print("Run one of the train_sim_tc*.py scripts to generate them.")
        else:
    ...
    for formula in compounds:
        ...
        if args.all:
            ...
            # RE-Free models do not extrapolate across the RE boundary. All is valid for both.
