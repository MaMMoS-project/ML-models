# -*- coding: utf-8 -*-
"""Train models to predict experimental Tc from compound embeddings.
...
Input files (outputs/):
    Experimental_Tc_RE-Free_w_embeddings_PCA.pkl
...
Each pkl must contain at least:
    ...
    Tc_exp                   – target variable (float, K)
...
Outputs:
    results/exp_tc_comparison.csv
    results/exp_tc_best_by_dataset.csv
    results/figures/<dataset>_<embedding>_<model>.png
    logs/train_exp_tc.txt
"""
...
print(f"[train_exp_tc] using n_jobs={_N_JOBS} for joblib/loky parallelism")
...
DATASETS = [
    {
        "name":  "RE-Free",
        "pca":   "Experimental_Tc_RE-Free_w_embeddings_PCA.pkl",
        "plain": "Experimental_Tc_RE-Free_w_embeddings.pkl",
    },
    {
        "name":  "RE",
        "pca":   "Experimental_Tc_RE_w_embeddings_PCA.pkl",
        "plain": "Experimental_Tc_RE_w_embeddings.pkl",
    },
    {
        "name":  "All",
        "pca":   "Experimental_Tc_all_w_embeddings_PCA.pkl",
        "plain": "Experimental_Tc_all_w_embeddings.pkl",
    },
]
...
def save_figure(
    ...
    for ax, yt, yp, label, color in [
        ...
        ax.set_xlabel("Tc_exp (K)")
        ax.set_ylabel("Tc_pred (K)")
        ax.set_title(label)
        ax.text(
            0.05, 0.95,
            f"R²  = {m['R2']:.3f}\nMAE = {m['MAE']:.1f} K\nRMSE= {m['RMSE']:.1f} K",
...
def train_one_dataset(ds: Dict, figures_dir: Path) -> List[Dict]:
    ...
    for emb_label, col_name in EMB_VARIANTS.items():
        ...
        y = df["Tc_exp"].values.astype(float)
...
def update_global_summary() -> None:
    ...
    writes the combined ``exp_tc_comparison.csv`` and ``exp_tc_best_by_dataset.csv``.
    ...
    out_csv = RESULTS_DIR / "exp_tc_comparison.csv"
    ...
    best_csv = RESULTS_DIR / "exp_tc_best_by_dataset.csv"
    ...
    print(f"Per-dataset tables   : {RESULTS_DIR}/<dataset>_results.csv")
...
if __name__ == "__main__":
    log_path = PROJECT_ROOT / "logs" / "train_exp_tc.txt"
