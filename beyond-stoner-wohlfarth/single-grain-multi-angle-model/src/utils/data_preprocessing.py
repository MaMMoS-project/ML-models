...
def _sph_to_cart(theta, phi):
    """
    Vectorized spherical (physics convention) -> Cartesian.
...
def make_reduced_dataset(df: pd.DataFrame,
                      angles_in_degrees: bool = False,
                      cube_edge_m: float = 50e-9) -> pd.DataFrame:
    """
    Adds physics-guided and orientation-aware features for uniaxial K1.
    Expects columns: ['Ms','A','K1','k1_theta','k1_phi','h_theta','h_phi'].
    Leaves the original df unchanged; returns a new df with added columns.
    """
    required = ['Ms','A','K1','k1_theta','k1_phi','h_theta','h_phi']
    ...
    if angles_in_degrees:
        th_k = np.deg2rad(out['k1_theta'].to_numpy(dtype=float))
        ph_k = np.deg2rad(out['k1_phi'  ].to_numpy(dtype=float))
        th_h = np.deg2rad(out['h_theta' ].to_numpy(dtype=float))
        ph_h = np.deg2rad(out['h_phi'   ].to_numpy(dtype=float))
    else:
        th_k = out['k1_theta'].to_numpy(dtype=float)
        ph_k = out['k1_phi'  ].to_numpy(dtype=float)
        th_h = out['h_theta' ].to_numpy(dtype=float)
        ph_h = out['h_phi'   ].to_numpy(dtype=float)

    # Unit vectors of easy axis and field direction
    uk_x, uk_y, uk_z = _sph_to_cart(th_k, ph_k)
    uh_x, uh_y, uh_z = _sph_to_cart(th_h, ph_h)

    # Relative orientation (uniaxial symmetry: ±u_k equivalent)
    dot_signed = uk_x*uh_x + uk_y*uh_y + uk_z*uh_z
    cos_relative_angle = np.clip(np.abs(dot_signed), 0.0, 1.0)  # unsigned angle
    relative_angle = np.arccos(cos_relative_angle)




    out = out.drop('k1_theta', axis=1)
    out = out.drop('k1_phi', axis=1)
    out = out.drop('h_theta', axis=1)
    out = out.drop('h_phi', axis=1) 

    out.insert(6,'relative_angle',relative_angle)


    # Physics scalars / dimensionless groups
    Ms = out['Ms'].to_numpy(dtype=float)
    A  = out['A' ].to_numpy(dtype=float)
    K1 = out['K1'].to_numpy(dtype=float)
    ##L  = float(cube_edge_m)

    # Guard against zero/negative where not defined
    safe_Ms = np.where(Ms == 0, np.nan, Ms)
    safe_Ms2 = np.where(Ms == 0, np.nan, Ms**2)
    safe_K1 = np.where(K1 <= 0, np.nan, K1)
