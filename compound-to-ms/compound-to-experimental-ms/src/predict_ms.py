#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Predict saturation magnetisation (Ms, experimental) for a new compound using a trained ONNX model.
...
(optional scaler) → regression model → log1p(Ms), inverted to Ms in A/m.  The user therefore only
...
python src/predict_ms.py --compound Fe3Pt --model results/onnx_models/All_raw_200D_rf_refeats_e0.onnx

# Run all available ONNX models and print a comparison table
python src/predict_ms.py --compound Fe3Pt --all

# Predict using only the best model for the detected compound type (RE or RE-Free)
python src/predict_ms.py --compound Fe3Pt --best
python src/predict_ms.py --compound Nd2Fe14B --best

# Predict a list of compounds (one formula per line)
python src/predict_ms.py --compounds-file compounds.txt --all
...
BEST_CSV     = PROJECT_ROOT / "results" / "exp_ms_best_by_dataset.csv"
...
# Printed to stderr before predictions (unless --no-disclaimer). See README / the
# "What the target actually is" note: the model learns a DFT collinear (fully-aligned)
# moment, which equals Ms only for ferromagnets and over-estimates ferrimagnets.
DISCLAIMER = """\
================================================================================
  DISCLAIMER - please read before using these predictions
--------------------------------------------------------------------------------
  This model predicts saturation magnetisation (Ms) from a compound's formula.
  Its training data is overwhelmingly DFT-derived and was computed assuming
  COLLINEAR FERROMAGNETIC order, so the learned quantity is really the
  fully-aligned magnetic moment:
    * for genuine FERROMAGNETS  -> it equals the physical Ms (reliable);
    * for FERRIMAGNETS / antiferromagnets / non-collinear magnets -> it is the
      UNCOMPENSATED sublattice sum, typically SEVERAL TIMES TOO LARGE
      (e.g. magnetite Fe3O4, spinel ferrites, garnets/YIG, hexaferrites).
  Ferrimagnetism CANNOT be identified from composition alone, so the training
  set most likely still contains unreliable simulated Ms values for unknown
  ferrimagnetic compounds, and this model may silently OVER-PREDICT Ms for them
  (oxides / ferrites in particular).
  => Treat every prediction as PROVISIONAL. Verify against experiment or a
     proper magnetic-structure calculation before relying on it, and use the
     results with great care - especially for oxides, ferrites, garnets, and
...
def _load_best_model_tags() -> "dict[str, str]":
    """Return {onnx_base_name: 'best for <Dataset>'} from exp_ms_best_by_dataset.csv.
...
def predict_with_model(onnx_path: Path, emb: np.ndarray,
    ...
    """Run ONNX inference and return scalar Ms prediction (A/m).
    ...
    # Ms models are trained on log1p(Ms); invert back to A/m.
    return float(np.expm1(np.squeeze(result)))
...
def _print_table(
    ...
    ms_headers = "".join(f"  {'Ms' + str(i+1) + ' (A/m)':>12}" for i in range(max_ensemble))
    print(f"{'Model':<55}  {'mean Ms (A/m)':>13}  {'Std (A/m)':>11}{ms_headers}")
    print("-" * (83 + 14 * max_ensemble))
    for base, member_preds in rows:
        n = len(member_preds)
        mean_ms = float(np.mean(member_preds))
        std_ms  = float(np.std(member_preds)) if n > 1 else float("nan")
        tag     = f"  [{best_tags[base]}]" if base in best_tags else ""
        label   = f"{base}{tag}"
        ms_vals = "".join(f"  {ms:>12.4g}" for ms in member_preds)
        ms_vals += "".join(f"  {'':>12}" for _ in range(max_ensemble - n))
        std_str = f"{std_ms:>11.4g}" if n > 1 else f"{'':>11}"
        print(f"{label:<55}  {mean_ms:>13.4g}  {std_str}{ms_vals}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Predict Ms from a compound formula using a trained ONNX model.",
    ...
    parser.add_argument(
        "--no-disclaimer",
        action="store_true",
        help="Suppress the ferrimagnet / DFT-Ms reliability disclaimer (printed to stderr).",
    )
    args = parser.parse_args()

    if not args.no_disclaimer and not args.list:
    ...
    if args.list:
        ...
        if not groups:
            ...
            print("Run one of the train_ms*.py scripts to generate them.")
        else:
    ...
    for formula in compounds:
        ...
        if args.all:
            ...
            # the RE model gives a badly wrong Ms). The All model is valid for both.
