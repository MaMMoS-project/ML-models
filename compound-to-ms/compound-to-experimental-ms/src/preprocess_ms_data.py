#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Pre-process the raw sources into the EXPERIMENTAL saturation-magnetisation dataset.

Stage-1 of the compound-to-experimental-ms pipeline (analogous to
``compound-to-experimental-tc/src/process_tc_data.py``, but for Ms and specialised to the
experimental target only — this project trains a model that predicts Ms directly from a
compound formula).

It reads the raw EXPERIMENTAL Ms sources from ``data/``, converts every source to a common
unit (ampere/metre, A/m), pools all values for a composition and reduces them with a SINGLE
median (matching my_ms/src/build_merged_dataset.py and the compound-to-tc projects — no
...
Ms is the experimental saturation magnetisation in A/m.

Raw EXPERIMENTAL sources (in data/) and their conversion to A/m:
    literature_values.csv     sep=';'  'Compound'  'mu0Ms (T)'          / MU_0
    Bhandari_I_exp.csv        sep='|'  'Material'  'Ms_exp (MA/m)'      * 1e6
    Bhandari_XIII_exp.csv     sep='|'  'Material'  'Ms (MA/m)'          * 1e6
    mp_fm_dedup_exp_data.csv  csv      'formula_pretty'
...
KEY = "composition"
MS_COL = "Ms"                # experimental saturation magnetisation [A/m]
...
def load_literature(d: Path) -> pd.DataFrame:
    df = pd.read_csv(d / "literature_values.csv", sep=";")
    return _frame(df["Compound"], df["mu0Ms (T)"] / MU_0)


def load_bhandari_i(d: Path) -> pd.DataFrame:      # MA/m
    df = pd.read_csv(d / "Bhandari_I_exp.csv", sep="|")
    return _frame(df["Material"], df["Ms_exp (MA/m)"] * 1e6)


def load_bhandari_xiii(d: Path) -> pd.DataFrame:   # MA/m
    df = pd.read_csv(d / "Bhandari_XIII_exp.csv", sep="|")
    return _frame(df["Material"], df["Ms (MA/m)"] * 1e6)


def load_mp_exp(d: Path) -> pd.DataFrame:          # mu_B / Angstrom^3 -> A/m
    df = pd.read_csv(d / "mp_fm_dedup_exp_data.csv",
...
def process(data_dir: Path, ms_threshold: float = 50_000.0,
    ...
    """Pool all experimental sources, canonicalise each formula to its reduced formula so
    ...
    pooled = pd.concat([ld(data_dir) for ld in EXP_LOADERS], ignore_index=True)
...
def main() -> None:
    ...
    print(f"Experimental Ms  — {len(df)} compositions "
