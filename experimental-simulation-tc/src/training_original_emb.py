# -*- coding: utf-8 -*-
"""Train models on original data WITH EMBEDDINGS.

This script is intended to be run from the project root:

    python src/training_original_emb.py

It trains models on the original (non-augmented) All-Pairs dataset WITH embeddings:
...
The script requires the outputs from the embedding and compression pipeline:
    ...
    src/out/Pairs_all_emb_w_embeddings.pkl
...
def train_original_embedding():
    ...
    dataset_file_map = {
        "all": "Pairs_all_emb_w_embeddings",
        "re": "Pairs_RE_emb_w_embeddings",
        "re-free": "Pairs_RE_Free_emb_w_embeddings"
    }
    ...
    def load_dataset_for_type(dataset_type):
        ...
        if not loaded_dataframes[dataset_type]["loaded"]:
            ...
            try:
                ...
                # Filter to pairs if necessary
                if 'pair_exists' in df.columns:
                    before_len = len(df)
                    df = df[df['pair_exists'] == True].copy()
                    print(f"Filtered to pairs only: {before_len} -> {len(df)} samples")
    ...
    try:
        ...
        # Filter to pairs if necessary
        if 'pair_exists' in df_data.columns:
            before_len = len(df_data)
            df_data = df_data[df_data['pair_exists'] == True].copy()
            print(f"Filtered to pairs only: {before_len} -> {len(df_data)} samples")
    ...
    for config in dataset_configs:
        ...
        for embedding_type in embedding_types:
            ...
            try:
                ...
                # Monkey patch the load_pairs_data to return our DataFrame
                original_load_pairs = lin_trainer.loader.load_pairs_data
                ...
                lin_trainer.loader.load_pairs_data = original_load_pairs
            ...
            try:
                ...
                # Monkey patch the load_pairs_data to return our DataFrame
                original_load_pairs = rf_trainer.loader.load_pairs_data
                ...
                # Restore original method
                rf_trainer.loader.load_pairs_data = original_load_pairs
            ...
            try:
                ...
                # Monkey patch the load_pairs_data to return our DataFrame
                original_load_pairs = mlp_trainer.loader.load_pairs_data
                ...
                # Restore original method
                mlp_trainer.loader.load_pairs_data = original_load_pairs
                dataset_emb_results.append({
                    "Model_Family": "MLP",
                    "Model": "FCNN",
                    "Dataset": dataset_name,
    ...
    if all_results:
        ...
        out_dir = results_dir / "original_emb_comparison"
        out_dir.mkdir(parents=True, exist_ok=True)
        out_csv = out_dir / "original_emb_models_comparison.csv"
        df.to_csv(out_csv, index=False)

        print("\n" + "=" * 80)
        print("ORIGINAL DATA WITH EMBEDDINGS - MODEL COMPARISON")
        print("=" * 80)
        ...
        out_best_dataset_emb_csv = out_dir / "original_emb_best_by_dataset_embedding.csv"
        ...
        out_best_csv = out_dir / "original_emb_best_by_dataset.csv"
        ...
        if len(df) > 3:
            try:
                ...
                pivot_csv = out_dir / "original_emb_comparison_pivot.csv"
