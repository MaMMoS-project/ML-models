...
This script trains the existing model families on the All-Augm dataset
without using any embeddings, using the augmented CSV produced by
``augment_data.py`` at:

    src/out/Augm_all.csv
...
creates a comparison table for the augmented case.
"""
...
def training_augmented():
    ...
    # Create a custom DataLoader with the specific file names for augmented dataset
    custom_loader = DataLoader(
        augmented_file="Augm_all.csv",
        re_augmented_file="Augm_RE.csv",
        re_free_augmented_file="Augm_RE_Free.csv"
    )
    ...
    for config in dataset_configs:
        ...
        try:
            sr_trainer = SymbolicRegressionTrainer(
                output_dir=str(results_dir / "augmented_sr")
            )
            # Use custom loader
        ...
        try:
            lin_trainer = LinearModelsTrainer(
                output_dir=str(results_dir / "augmented_linear")
            )
            # Use custom loader
        ...
        try:
            rf_trainer = RandomForestTrainer(
                output_dir=str(results_dir / "augmented_rf")
            )
            # Use custom loader
        ...
        # 4. FCNN/MLP        # 4. FCNN/MLP
        try:
            mlp_trainer = FCNNTrainer(
                output_dir=str(results_dir / "augmented_fcnn")
            )
            # Use custom loader
    ...
    if all_results:
        ...
        out_dir = results_dir / "augmented_comparison"
        out_dir.mkdir(parents=True, exist_ok=True)
        out_csv = out_dir / "augmented_models_comparison.csv"
        df.to_csv(out_csv, index=False)

        print("\n" + "=" * 80)
        print("AUGMENTED DATA (NO EMBEDDINGS) - MODEL COMPARISON")
        print("=" * 80)
        ...
        out_best_csv = out_dir / "augmented_best_by_dataset.csv"
        ...
        try:
            ...
            pivot_csv = out_dir / "augmented_comparison_pivot.csv"
