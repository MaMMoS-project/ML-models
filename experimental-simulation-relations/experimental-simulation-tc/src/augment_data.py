...
class CurieTempAugmenter:
    ...
    @staticmethod
    def _load_column_from_csv(file_path, column: str) -> np.ndarray:
        """Load a named column from a MammoS CSV file, dropping NaN values."""
        try:
            df = pd.read_csv(file_path, skiprows=4)
            if 'info' in df.columns:
                df = df[df['info'].isna()]
            ...
            print(f"Error loading '{column}' from {file_path}: {e}")
            return np.array([])
    ...
    def _run_value_ks_tests(self, phase_title: str, splits: list) -> None:
        """
        Run KS tests comparing raw Tc_exp and Tc_sim distributions separately.

        Complements _run_phase_ks_tests (which compares Tc_delta) by testing
        whether the individual temperature values in the augmented dataset are
        statistically consistent with those in the original paired data.
        Both a standard KS test and a subsampling-based KS test are run for
        each column and each split.
        ...
        phase_title : str
            Label printed in the section headers.
        splits : list of (name, pairs_path, augm_path)
            Dataset splits to compare (typically RE and RE-free).
        """
        from scipy import stats

        for column in ('Tc_exp', 'Tc_sim'):

            # --- Standard KS test ---
            print("\n" + "="*60)
            print(f"STANDARD KS TEST ({column}) — {phase_title}")
            print("="*60)
            print(f"Comparing {column}:  paired data (Pairs_*.csv)  vs.  augmented dataset")
            ...
            for i, (name, orig_path, augm_path) in enumerate(splits, 1):
                ...
                if not Path(orig_path).exists() or not Path(augm_path).exists():
                    print("  Skipping: missing files")
                    continue
                orig = self._load_column_from_csv(orig_path, column)
                augm = self._load_column_from_csv(augm_path, column)
                if len(orig) == 0 or len(augm) == 0:
                    print(f"  Skipping: '{column}' data unavailable")
                    continue
                ks_stat, p_value = stats.ks_2samp(orig, augm)
                alpha = 0.05
                print(f"  Original: {len(orig)} values  |  Augmented: {len(augm)} values")
                print(f"  KS statistic: {ks_stat:.4f}  |  p-value: {p_value:.8f}")
                print(f"  Original:  mean={np.mean(orig):.2f} K, std={np.std(orig):.2f} K")
                print(f"  Augmented: mean={np.mean(augm):.2f} K, std={np.std(augm):.2f} K")
                if p_value < alpha:
            ...
            # --- Subsampling KS test ---
            print("\n" + "="*60)
            print(f"LARGE-SAMPLE KS TEST ({column}, SUBSAMPLING) — {phase_title}")
            print("="*60)
            print(f"Subsampling-based test for {column} — robust to large n")
    ...
    def _write_mammos_csv(self, path: Path, df: pd.DataFrame) -> None:
        ...
        # Write the Mammos header — metadata rows must have exactly as many
        # comma-separated fields as the data has columns (mammos_entity enforces this).
        n_cols = len(simplified_df.columns)
        meta_row = "#" + "," * (n_cols - 1) + "\n"
        header_lines = [
            "#mammos csv v1\n",
            meta_row,
