# -*- coding: utf-8 -*-
"""Train baseline models on Phase 3 combined augmented data (no embeddings).

Usage:
    python -m src.training_augmented

Requires: outputs/Augm_combined_*.csv (run src/augment_data.py first).
"""
...
def training_augmented():
    ...
    if all_results:
        ...
        df.to_csv(out_dir / "augmented_models_comparison.csv", index=False)
        ...
        best_by_dataset.to_csv(out_dir / "augmented_best_by_dataset.csv", index=False)

        try:
            pivot_df = df.pivot_table(
                index="Dataset", columns="Model_Family", values="R2", aggfunc='max'
            ).reset_index()
            pivot_df.to_csv(out_dir / "augmented_comparison_pivot.csv", index=False)
