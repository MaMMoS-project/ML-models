...
def is_valid_composition(comp):
    """Return True if composition looks like a valid chemical formula.""" # TODO: fine tune this function also for emb later perhaps
...
def remove_small_ms_values(df, sim_col, exp_col, threshold):
    print('DF shape before thresholding:', df.shape[0])
    # df_filtered = df[(df[exp_col] > threshold) & (df[sim_col] > threshold)]
    df_filtered = df[
    (
        (df[exp_col] > threshold) & (df[sim_col] > threshold)
    ) | 
    (
        df[exp_col].isna() | df[sim_col].isna())]
    print('DF shape after threshoding:', df_filtered.shape[0])
    return df_filtered

def filter_pairs(df, sim_col, exp_col):
    print('DF shape before extracting pairs:', df.shape[0])
    df_filtered = df[df[sim_col].notna() & df[exp_col].notna()].copy()
    print('DF shape after extracting pairs:', df_filtered.shape[0])
    return df_filtered
